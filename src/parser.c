...
static ScopeNode* newScope(ScopeNode* parent)
{
...
static void freeScope(ScopeNode* node)
{
...
node* block_stmt(Parser* parser);

node* declaration(Parser* parser);
...
Ty* resolve_type(Parser* parser, bool isDeclared)
{
    ...
    while(parser->next.type == TYPE || parser->next.type == IDENTIFIER)
    {
...
static node* func_decl(Parser* parser, Ty* returnType)
{
    ...
    while(parser->current.type != CLOSE_PAREN)
    {
        ...
        result->paramCount++;
        if(parser->current.type == COMMA) advance(parser);
    }
    ...
    if(parser->current.type == BRACE) result->body = block_stmt(parser);
    else result->body = NULL;

    return (node*) result;
}

static node* struct_or_union(Parser* parser)
{
...
static node* typedef_decl(Parser* parser)
{
...
static node* class_decl(Parser* parser)
{
    ...
    while(parser->current.type != CLOSE_BRACE)
    {
        ...
        node* func = func_decl(parser, type);
...
node* declaration(Parser* parser)
{
    switch(parser->current.type)
    {
        case TYPE:
        {
            ...
            {
                case PAREN: return func_decl(parser, type);
                default: return var_decl(parser, type);
            }
            break;
        }
        case STRUCT:
        case UNION: 
            advance(parser);
            return struct_or_union(parser);
        case TYPEDEF: return typedef_decl(parser);
        case CLASS: return class_decl(parser);
        default: return statement(parser);
    }
}
...
static node* switch_stmt(Parser* parser)
{
    SwitchStatementNode* result = malloc(sizeof(SwitchStatementNode));
            ...
            do {
                ...
                current->caseStmt = statement(parser);
...
static node* for_stmt(Parser* parser)
{
    ...
    result->loopStmt = statement(parser);
    return (node*) result;
}
...
static node* if_stmt(Parser* parser)
{
    ...
    consume(parser, CLOSE_PAREN, "expected ')' after expression!");
    result->thenStmt = statement(parser);
    if(parser->current.type == ELSE)
    {
        advance(parser);
        result->elseStmt = statement(parser);
    }
    else 
    {
...
node* block_stmt(Parser* parser)
{
    ...
    while(parser->current.type != CLOSE_BRACE)
    {
        node* current = declaration(parser);
...
node* statement(Parser* parser)
{
    switch(parser->current.type)
    {
        case SWITCH: return switch_stmt(parser);
        case FOR: return for_stmt(parser);
        case IF: return if_stmt(parser);
        case WHILE: return while_stmt(parser);
        case BRACE: return block_stmt(parser);
        case RETURN: return return_stmt(parser);
        case BREAK:
        {
            advance(parser);
...
node* field_call(Parser* parser, node* expression1)
{
    ...
    n->_struct = expression1;
    n->op = parser->previous;
    n->field = expression(parser);
    return (node*)n;
}
...
void printNodes(node* start, int depth)
{
    ...
    switch(start->nodeType)
    {
        ...
        case NODE_VARIABLEDECL:
        {
            ...
            printf("Type: Variable Declaration; Name: %.*s Type: %s\n", n->identifier.length, n->identifier.start, typeToString(n->type));
            printf("Value: \n");
            if(n->value != NULL) printNodes(n->value, depth + 1);
            else printf("NULL\n");
            break;
        }
        case NODE_FUNCTIONDECL:
        {
            ...
            printf("Type: Function Declaration; Name: %.*s, Return Type: %s\n", n->identifier.length, n->identifier.start, typeToString(n->returnType));
            printf("Parameters: ");
            bool first = true;
        ...
        {   
            ...
            printf("Type: Typedef; Aliased Type: %s Defined Type: %s\n", typeToString(n->typeAliased), typeToString(n->typeDefined));
            break;
        }
        case NODE_RECORDDECL:
        {
            RecordDeclarationNode* n = (RecordDeclarationNode*)start;
            printf("Type: %s; Name: %s\n", n->struct_or_union == IS_STRUCT ? "STRUCT" : n->struct_or_union == IS_UNION ? "UNION" : "UNDEFINED", typeToString(n->typeDefined));
            bool first = true;
            printf("Fields: ");
            for(size_t i = 0; i < n->fieldCapacity; i++)
            {
                ...
                printf("%s %.*s", typeToString(n->fields[i].type), n->fields[i].identifier.length, n->fields[i].identifier.start);
            }
            printf("\n");
            break;
        }
        ...
        case NODE_FIELDCALL:
        {
            ...
            printNodes(n->_struct, depth + 1);
            printNodes(n->field, depth + 1);
            break;
        }
...
ProgramNode* parse(const char* src)
{
    ...
    while(parser.current.type != NEWLINE)
    {
        ...
        ast->declarations[declarationCount] = declaration(&parser);
        declarationCount++;
    }
